...
class Git extends EventEmitter {
  ...
  */
   ...
   * @param  {Array}   options     - the https options
  ...
  listen(port, options, callback) {
    ...
    this.server.listen(port, function(error, result) {
      callback(error, result, port, 'http');
    });
    ...
    if(typeof options != 'function') {
      ...
      this.server.listen(options.port, function(error, result) {
        callback(error, result, options.port, 'https');
      });
    }
  }
  /**
