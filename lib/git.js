...
class Git extends EventEmitter {
  ...
  */
   ...
   * @method listen
   * @memberof Git
   * @param  {String}   type     - the type of server you want. Can be either 'http' or 'https'
   * @param  {Array}   options     - the configuration of the webserver
   * @param  {Function} callback - the function to call when server is started or error has occured
   */
  listen(type, options, callback) {
    var self = this;
    if(typeof type != 'string') {
      callback("V0.4.2 Requires The First Argument Of Listen To Be An Array Of Options. Please Check The Examples To Update Your Code!");
    } else {
      if(type == 'http') {
        ...
        if(options.cert != undefined && options.key != undefined) {
          ...
          callback("A Certificate and Key Is Required To Start The HTTPS Server!");
        }
      } else {
        callback("A Varible 'type' need to be in the options and must be set to 'http' or 'https' to set the server type!");
      }
    }
  }
  /**
