...
class Git extends EventEmitter {
  ...
  */
  ...
  /**
   * Authenticate the user with a basic authentication
   * @method authenticate
   * @memberof Git
   * @param {Object}   req
   * @param {Object}   res
   ...
   * @param {Callback} next
  */
  authenticateWithBasicAuth(req, res, service, repoName, next) {
        ...
        function(req, res) {
            ...
            if(this.authenticate) {
                if(this.authenticationHandler) {
                    this.authenticationHandler(service === 'upload-pack' ? 'upload' : 'download', repoName, (doAuthenticate) => {
                        if(doAuthenticate) {
                          self.authenticateWithBasicAuth(req, res, service, repoName, next);
                        } else next();
                    });
                } else {
                    self.authenticateWithBasicAuth(req, res, service, repoName, next);
