...
class Git extends EventEmitter {
  ...
  */
   ...
   * @param  {Boolean=}  options.cors - enable allow-all CORS headers
  ...
  listen(port, options, callback) {
      ...
      this.server = createServer(function(req, res) {
        if (options.cors) {
          ...
          // For an OPTIONS request, we should never return a body, so we write
          // a 200 and then close the request here.
